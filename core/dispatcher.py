...
# README 明确规定: 噪声行的字段值为 "N/A_数字" 形式, 直接跳过。
...
# 字段名强烈提示为"单实体结构化字段"时，跳过 is_unstructured_text 直接走 structured
_STRUCTURED_FIELD_RE = re.compile(
    ...
    r"consignee|holder_name|patient_name|"         # [新增] 收货人 / 持有人 / 患者
    r"xingming|xm|"
    ...
    r"vin|vin_code|"                               # [新增] VIN
    r"hf_no|ss_no|medical_record_no"               # [新增] 公积金/社保/病历号
    r")\b",
    re.IGNORECASE,
)
...
def dispatch(field_name: str, value, record_id, table_name: str,
    ...
    # ── 5a. 字段名=encoded/payload/base64 但 value 不是真编码:
    #       走 structured 正则扫描后把 data_form 重标成 encoded
    if _ENCODED_FIELD_RE.search(field_name):
        # 先尝试 encoded 扫描器的字段名强信任路径(scan_encoded_field 内部会判断)
        hits = scan_encoded_field(field_name, value_str, record_id,
                                   table_name, field_name, db_type, db_name)
        if hits:
            return hits
        # 兜底:走 structured + 重标 data_form
    ...
    # ── 5b. 字段名=单实体结构化字段 → 强制走 structured ────────────
