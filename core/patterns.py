...
NAME_BLACKLIST = {
    ...
    "方当事", "方当事人", "公输翰", "公输翰翮", "公输鹰",
    "共叔傲", "共叔傲云",

    # 新增:来自 diff_report.txt FP 样本的高频普通词误判名字
    "家长", "钱包", "武胜", "石散化", "乐身", "步核",
    "家住", "家中", "家里", "家有", "家门",
    "钱款", "钱财",
    "石化", "石膏", "石蜡",
...
REGEX_PATTERNS = {
    ...
    "VIN_CODE": re.compile(r"(?<![A-HJ-NPR-Z0-9])[A-HJ-NPR-Z][A-HJ-NPR-Z0-9]{16}(?![A-HJ-NPR-Z0-9])"),
    ...
    "ADDRESS": re.compile(
        ...
        r"(?:[\u4e00-\u9fa5]{2,6}[区县旗]\s*)?"
        r"(?:"
        # 分支1:完整形式 [中文街道/路/社区/开发区/…]NNN号NNN室
        # body 上限由 15→25:应对"合肥新站高新技术产业开发区七里塘"(16字)等长地名。
        # 新增社区/开发区后缀:覆盖"滨湖世纪社区"/"经济技术开发区"等场景。
        # 新增"旗"到区县字符集:覆盖内蒙古"土默特左旗"等旗级行政区。
        r"[\u4e00-\u9fa5]{2,25}(?:街道|大道|路|街|乡|镇|村|道|社区|开发区)\d{1,5}号\d{1,5}室"
        r"|"
        # 分支2:半完整 [中文街道/路]\d+号(无室)
        r"[\u4e00-\u9fa5]{2,25}(?:街道|大道|路|街|乡|镇|村|道|社区|开发区)\d{1,5}号"
        r"|"
        # 分支3:街道/路名结尾(无号码)
