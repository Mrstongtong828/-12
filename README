...
- **差异分析**:按赛题 §6 公式(分桶 F1 + 加权 + 等级准确率 + 综合识别得分)复现靶机评分逻辑,通过 `diff-script.py` 对比 `test/example.csv` 样本,输出 FP/FN 明细与分层 F1。
...
│   ├── card_roi.py                # 证件图片区域检测与裁剪预处理
│   ├── ocr_client.py              # OCR 主进程客户端(熔断 + 超时)
│   ├── ocr_worker.py              # OCR 子进程(PaddleOCR v4 中文模型)
│   └── dbobject.py                # 存储过程 / 函数 / 触发器 / 视图扫描
├── scripts/                       # 调试与辅助脚本
├── test/                          # 测试目录(存放 example.csv 用于本地评分)
├── output/                        # upload.csv 输出目录
├── docs/交付物/                   # 作品文档(系统架构、脱敏方案、操作手册等)
├── models/paddleocr/              # PaddleOCR 离线模型
├── main.py                        # 主程序入口
├── diff-script.py                 # upload vs example 差异分析
├── docker-compose.yml             # 赛题数据库容器启动配置
├── permissions.yaml               # 权限管控策略配置
...
`docker-compose.yml` 已在项目根目录,加载赛题镜像后直接启动:
...
- UIE NER(可选):schema 裁剪到强结构先验实体(手机/身份证/银行卡/邮箱),按 500 字分块推理,AI 信号量限流
- 后置过滤:
...
- **多路径扫描**:原文本 + 纠错后文本 + 去空格等多路并行扫描,合并去重
...
| 维度         | 策略                                                                                            |
| ------------ | ----------------------------------------------------------------------------------------------- |
| 总时间预算   | 30 分钟硬时限,单表 540s 预算,超时即 break 继续下一张表                                         |
| 大表保护     | 行数估算(MySQL `TABLE_ROWS` / PG `reltuples`)> 5 万启用分层抽样(头/中/尾三段)                |
| BLOB 表保护  | 含 BLOB/BYTEA 列的表限扫 800 行,且排到所有普通表之后,防 OCR 熔断影响其他形态                   |
| BLOB 表过滤  | SQL 层按 `OCTET_LENGTH > 200` 过滤 NULL / 占位空 BLOB,避免预算浪费在空行上                      |
| 数据库并发   | `DB_WORKERS=2`,MySQL 与 PostgreSQL 线程并行扫描                                                 |
| OCR 并发池   | `OCR_POOL_SIZE=2`,两个 PaddleOCR 子进程构成池,BLOB 表内行级并发派发                            |
| OCR 隔离     | PaddleOCR 独立子进程,单图超时 38s + 连续失败 4 次熔断 + 周期性重建(每 50 次调用销毁重建)     |
| 内存控制     | 每个 OCR 子进程 `FLAGS_fraction_of_cpu_memory_to_use=0.20`(两个合计 ~6.4GB),给主进程/OS 留余量 |
| 图片预处理   | 长边压到 800px、总像素 ≤ 64 万,极端宽高比 >3 的额外压到 640px,削平 OCR rec 端 padding 峰值     |
...
`core/masking.py` 为 20 种敏感类型各配一条策略,核心设计:
...
```bash
python core/masking.py                           # 处理 output/upload.csv → output/masked_report.csv
python core/masking.py PHONE_NUMBER 13812345678  # 单值测试
```
...
准备 `test/example.csv`(赛题提供的约 1% 答案样本)后,可使用 `diff-script.py` 进行差异分析(见 10.2)。
...
对照 `output/upload.csv` vs `test/example.csv`,按形态 / 类型分组列出 FN 全量清单、FP top-20 字段名、分层 F1、等级准确率等,定位回归点。

### 10.3 日志

- **终端**:rich 实时进度条(扫描表名、行数、命中数、耗时)
- **文件**:`output/scan_error.log` 收集 WARNING/ERROR,不污染终端
...
A:去重键为 7 元组精确匹配(位置 + 类型 + 值),仅删除重复上报,同一值的多种类型标注保留首次发现。若仍存在,检查是否抽样扫描与全量扫描混用导致 `record_id` 不稳定。

**Q:BLOB 表扫了但几乎没命中?**

A:确认 PaddleOCR 模型已下载到 `./models/paddleocr/` 且三个子模型目录齐全。观察 `output/scan_error.log` 中是否有 OCR 连续失败的告警,若有需检查图片尺寸是否超出处理上限(自动压缩到 ≤ 800px 长边)。
...
3. `core/masking.py` 的 `MASK_STRATEGY` 补脱敏策略(可选)
...
详见 [`docs/交付物/`](docs/交付物/) 目录下的各文档,赛题原文见 [`../官方文档/`](../官方文档/)。
